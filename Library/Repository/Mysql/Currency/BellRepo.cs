...
public class BellRepo : ICurrencyRepo, IPayableCurrency
{
    public CurrencyType CurrencyType { get; } = CurrencyType.Bell;
    private readonly CurrencyCommonRepo _currency = CurrencyCommonRepo.Of();

    public static BellRepo Of() => new BellRepo();
    private BellRepo()
    {        
    }
    ...
    public Task<(ErrorCode, long)> DecreaseAsync(MySqlConnection db, MySqlTransaction transaction, ItemGainReason reason, ulong userSeq, long itemPrice)
    {
