...
public class BackGroundPropItemPolicy : IItemPolicy, IDistributableItem, IInventoryHasItem, IEquipableItem, IVisualItem
{
    public MainItemType ItemType { get; } = MainItemType.BackGroundProp;
    public string TblName { get; } = "tbl_inventory_backgroundprop";
    private ItemSharedRepo? _itemRepository;

    public BackGroundPropItemPolicy()
    {
    ...
    public async Task<int> FetchHasCountAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId)
    {
        ...
        var count = await _itemRepository.FetchHasCountAsync<TblInventoryBackgroundProp>(db, transaction, charSeq, itemId);
        return count;
    }
    ...
    public async Task<InventoryItem> FetchInventoryItemAsync(MySqlConnection db, ulong charSeq, ulong itemSeq)
    {
        ...
        var tblItem = await _itemRepository.FetchInventoryItemAsync<TblInventoryBackgroundProp>(db, charSeq, itemSeq);
        if (tblItem == null)
            return new InventoryItem();
    ...
    public async Task<List<InventoryItem>> FetchInventoryAllItemsAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq)
    {
        ...
        var tblItems = await _itemRepository.FetchInventoryAllItemsAsync<TblInventoryBackgroundProp>(db, transaction, charSeq);
    ...
    public List<InventoryItem> FetchInventoryAllItems(MySqlConnection db, ulong charSeq)
    {
        ...
        var tblItems = _itemRepository.FetchInventoryAllItems<TblInventoryBackgroundProp>(db, charSeq);
    ...
    public async Task<InventoryItem> FetchItemByIdAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId)
    {
        ...
        var tblItem = await _itemRepository.FetchItemByIdAsync<TblInventoryBackgroundProp>(db, transaction, charSeq, itemId);
        if (tblItem == null)
            return new InventoryItem();
    ...
    public async Task<ErrorCode> UpdateInventoryEquipedAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId, ulong itemSeq, int slotId, short equipItem)
    {
        // 기존 아이템들 0으로 설정        
        {
