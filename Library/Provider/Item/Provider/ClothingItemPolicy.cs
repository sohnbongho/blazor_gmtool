...
public class ClothingItemPolicy : IItemPolicy, IRewardableItem, IDistributableItem, IInventoryHasItem, ITimeExtendableItem, IDyeableItem, IEquipableItem, IWearableItem, IExpiredItem
{
    public MainItemType ItemType { get; } = MainItemType.Clothing;
    public string TblName { get; } = "tbl_inventory_clothing";
    private ItemSharedRepo? _itemRepository;

    public ClothingItemPolicy()
    {
    ...
    public async Task<(ErrorCode, string, DateTime)> DistributeItemAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId, int slotId, ulong itemSeq, int amount, int itemPeriod, int colorId)
    {
        ...
        var result = await _itemRepository.DistributeItemAsync<TblInventoryClothing>(db, transaction, charSeq, itemId, slotId, itemSeq, amount, itemPeriod, colorId);
        return result;
    }
    ...
    public (ErrorCode, string, DateTime) DistributeItem(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId, int slotId, ulong itemSeq, int amount, int itemPeriod, int colorId)
    {
        ...
        var result = _itemRepository.DistributeItem<TblInventoryClothing>(db, transaction, charSeq, itemId, slotId, itemSeq, amount, itemPeriod, colorId);
        return result;
    }
    ...
    public async Task<(ErrorCode, string, DateTime, ulong)> ExtendItemUsagePeriod(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId, int itemPeriod, int colorId)
    {
        ...
        var (errorCode, query, expired, tbl) = await _itemRepository.ExtendItemUsagePeriodAsync<TblInventoryClothing>(db, transaction, charSeq, itemId, itemPeriod, colorId);
    ...
    public async Task<InventoryItem> FetchItemByIdAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId)
    {
        ...
        var tblItem = await _itemRepository.FetchItemByIdAsync<TblInventoryClothing>(db, transaction, charSeq, itemId);
        if (tblItem == null)
            return new InventoryItem();
    ...
    public async Task<InventoryItem> FetchInventoryItemAsync(MySqlConnection db, ulong charSeq, ulong itemSeq)
    {
        ...
        var tblItem = await _itemRepository.FetchInventoryItemAsync<TblInventoryClothing>(db, charSeq, itemSeq);
        if (tblItem == null)
            return new InventoryItem();
    ...
    public async Task<List<InventoryItem>> FetchInventoryAllItemsAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq)
    {
        ...
        var tblItems = await _itemRepository.FetchInventoryAllItemsAsync<TblInventoryClothing>(db, transaction, charSeq);
    ...
    public List<InventoryItem> FetchInventoryAllItems(MySqlConnection db, ulong charSeq)
    {
        ...
        var tblItems = _itemRepository.FetchInventoryAllItems<TblInventoryClothing>(db, charSeq);
    ...
    public async Task<InventoryItem> FetchPrevEquippedItemAsync(MySqlConnection db, ulong charSeq, ulong itemSeq, int slotId)
    {
        ...
        var tblItem = await _itemRepository.FetchPrevEquippedItemAsync<TblInventoryClothing>(db, charSeq, itemSeq, slotId); ;
        if (tblItem == null)
            return new InventoryItem();
    ...
    public async Task<ErrorCode> UpdateInventoryEquipedAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId, ulong itemSeq, int slotId, short equipped)
    {
        ...
        var errorCode = await _itemRepository.UpdateInventoryEquipedAsync<TblInventoryClothing>(db, transaction, charSeq, itemId, itemSeq, slotId, equipped);
        return errorCode;
    }
    ...
    public async Task<int> FetchHasCountAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId)
    {
        ...
        var count = await _itemRepository.FetchHasCountAsync<TblInventoryClothing>(db, transaction, charSeq, itemId);
        return count;
    }
    public async Task<(ErrorCode, string, DateTime)> DyeItem(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, ulong itemSeq, string jsonData, int colorId)
    {
        ...
        var (errorCode, query, expiredTime, tbl) = await _itemRepository.DyeItem<TblInventoryClothing>(db, transaction, charSeq, itemSeq, jsonData, colorId);
        if (errorCode != ErrorCode.Succeed)
        {
    ...
    public async Task<(ErrorCode, InventoryItem)> RewardItemAsync(MySqlConnection db, MySqlTransaction transaction, ulong userSeq, ulong charSeq, int itemId, int amount)
    {
        ...
        {
            // 기간만 늘린다.
            var (errorCode, query2, expireTime2, tbl) = await _itemRepository.ExtendItemUsagePeriodAsync<TblInventoryClothing>(db, transaction, charSeq, itemId, itemPeriodDay, colorId);
            if (errorCode != ErrorCode.Succeed)
        ...
        else
        {
            ...
            // 아이템 지급
            var (errorCode, query, expireTime2) = await _itemRepository.DistributeItemAsync<TblInventoryClothing>(db, transaction, charSeq, itemId, slotId, itemSeq, itemAmount, itemPeriodDay, colorId);
    ...
    public (ErrorCode, InventoryItem) RewardItem(MySqlConnection db, MySqlTransaction transaction, ulong userSeq, ulong charSeq, int itemId, int amount)
    {
        ...
        if (hasCount > 0)
        {
            ...
            var (errorCode, query2, expireTime2, tbl)
                = _itemRepository.ExtendItemUsagePeriod<TblInventoryClothing>(db, transaction,
    ...
    public async Task<ErrorCode> HandleExpiredItem(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, DateTime now)
    {
        ...
        var errorCode = await _itemRepository.HandleExpiredItem<TblInventoryClothing>(db, transaction, charSeq, now);
        return errorCode;
    }
}
