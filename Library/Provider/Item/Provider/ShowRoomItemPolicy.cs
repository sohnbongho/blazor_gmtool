...
public class ShowRoomItemPolicy : IItemPolicy, IDistributableItem, IInventoryHasItem, IEquipableItem, IShowRoomItem
{
    public MainItemType ItemType { get; } = MainItemType.ShowRoom;
    public string TblName { get; } = "tbl_inventory_showroom";
    private ItemSharedRepo? _itemRepository;

    public ShowRoomItemPolicy()
    {
    ...
    public async Task<InventoryItem> FetchItemByIdAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId)
    {
        ...
        var tblItem = await _itemRepository.FetchItemByIdAsync<TblInventoryShowroom>(db, transaction, charSeq, itemId);
        if (tblItem == null)
            return new InventoryItem();
    ...
    public async Task<InventoryItem> FetchInventoryItemAsync(MySqlConnection db, ulong charSeq, ulong itemSeq)
    {
        ...
        var tblItem = await _itemRepository.FetchInventoryItemAsync<TblInventoryShowroom>(db, charSeq, itemSeq);
        if (tblItem == null)
            return new InventoryItem();
    ...
    public async Task<List<InventoryItem>> FetchInventoryAllItemsAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq)
    {
        ...
        var tblItems = await _itemRepository.FetchInventoryAllItemsAsync<TblInventoryShowroom>(db, transaction, charSeq);
    ...
    public List<InventoryItem> FetchInventoryAllItems(MySqlConnection db, ulong charSeq)
    {
        ...
        var tblItems = _itemRepository.FetchInventoryAllItems<TblInventoryShowroom>(db, charSeq);
    ...
    public async Task<List<InventoryItem>> FetchInventoryEquippedItemsAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, short equipped)
    {
        ...
        var tblItems = await _itemRepository.FetchInventoryEquippedItemsAsync<TblInventoryShowroom>(db, transaction, charSeq, equipped);
    ...
    public async Task<int> FetchHasCountAsync(MySqlConnection db, MySqlTransaction transaction, ulong charSeq, int itemId)
    {
        ...
        var count = await _itemRepository.FetchHasCountAsync<TblInventoryShowroom>(db, transaction, charSeq, itemId);
        return count;
    }
    ...
    public async Task<InventoryItem> FetchPrevEquippedItemAsync(MySqlConnection db, ulong charSeq, ulong itemSeq, int slotId)
    {
        ...
        var result = await db.QueryAsync<TblInventoryShowroom>(query, new
        {
            char_seq = charSeq,
            item_seq = itemSeq,
            equipped = 1,
